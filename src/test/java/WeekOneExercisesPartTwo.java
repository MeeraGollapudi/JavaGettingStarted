...
public class WeekOneExercisesPartTwo {
    ...
    private float calculateBAC(char gender, int weightInPounds, int ouncesOfAlcoholConsumed, float hoursSinceLastDrink) {
        ...
        float actualBAC;
        float weightQuotient;
        float peakBAC;
        float weightConstant = 5.14f;
        float menAlcoholDist = 0.73f;
        float womenAlcoholDist = 0.66f;
        weightQuotient = weightConstant / weightInPounds;
        if (gender == 'M') {
            peakBAC = (weightQuotient * ouncesOfAlcoholConsumed * menAlcoholDist);
        } else {
            peakBAC = (weightQuotient * ouncesOfAlcoholConsumed * womenAlcoholDist);
        }
        actualBAC = peakBAC - (hoursSinceLastDrink * 0.015f);
        return actualBAC;
    }
}